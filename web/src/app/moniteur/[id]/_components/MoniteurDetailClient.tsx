      ...
      <div className="relative bg-primary text-white overflow-hidden border-b border-white/5">
        ...
        <div className="relative z-10 container py-12 lg:py-20">
          ...
          <div className="grid lg:grid-cols-[1fr_auto] gap-8 lg:gap-12 items-end">
            ...
            {/* Identity fiche — three clear zones in one card:
                  1. eyebrow row that names the card
                  2. dominant metrics ("1 / 4")
                  3. primary CTA + secondary editor link
                Mobile-first, never spans full width.            */}
            <div
              className="animate-in fade-in slide-in-from-right-2 duration-500 fill-mode-both w-full max-w-xs sm:max-w-sm lg:w-auto lg:max-w-none lg:min-w-[320px]"
              style={{ animationDelay: '220ms' }}
            >
              <div className="relative rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.08] to-white/[0.02] backdrop-blur-md shadow-[0_12px_40px_rgba(0,0,0,0.25)] overflow-hidden">
                {/* Soft amber accent in the upper-right corner —
                    same hue as the rest of the hero, kept low
                    opacity so it reads as ambient lift, not branding
                    noise. */}
                <div className="pointer-events-none absolute -top-12 -right-12 w-36 h-36 bg-amber-400/15 blur-3xl rounded-full" />
                <div className="pointer-events-none absolute -bottom-16 -left-16 w-32 h-32 bg-blue-500/10 blur-3xl rounded-full" />
                ...
                <div className="relative p-5 sm:p-6 flex flex-col gap-5">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-white/55">
                    <Newspaper className="w-3 h-3 text-amber-300/80" />
                    Ce numéro
                  </div>

                  {/* Metrics — two big numerals separated by a thin
                      vertical rule. Centered cells for visual
                      balance. The vertical rule disappears when
                      ``page_count`` is null and Documents stands
                      alone. */}
                  <div
                    className={cn(
                  ...
                  {/* Primary CTA — solid amber pill so the action
                      reads as the page's "thing to do". Targets the
                      LexHaïti-branded PDF export (always public).
                      48px tall to clear the WCAG tap-target floor. */}
                  <a
                    href={`/api/v1/moniteur/issues/${issue.id}/export`}
                    download
                    className="group/cta relative flex items-center justify-center gap-2 h-12 rounded-xl bg-amber-400 text-slate-900 font-bold text-sm shadow-[0_4px_16px_rgba(251,191,36,0.35)] hover:bg-amber-300 hover:shadow-[0_6px_20px_rgba(251,191,36,0.45)] active:scale-[0.98] transition-all"
                  >
                    <Download className="w-4 h-4 shrink-0 transition-transform group-hover/cta:translate-y-0.5" />
                    <span className="uppercase tracking-wide">
                      Télécharger ce numéro
                    </span>
                  </a>

                  {/* Secondary action — signed-in only. Reads as a
                      quiet ghost link so it stays below the primary
                      CTA in the visual hierarchy. */}
                  {isSignedIn && issue.file_url && (
                    <a
                      href={`/api/v1/moniteur/issues/${issue.id}/scan`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group/scan inline-flex items-center justify-center gap-1.5 -mt-2 text-[11px] font-semibold text-white/55 hover:text-white transition-colors"
                      title="Réservé aux éditeurs"
                    >
                      <FileText className="w-3 h-3" />
                      <span>Voir le scan original</span>
                      <ArrowRight className="w-3 h-3 opacity-0 group-hover/scan:opacity-100 group-hover/scan:translate-x-0.5 transition-all" />
                    </a>
                  )}
                </div>
              </div>
...
function HeroStat({
  ...
  return (
    <div className="px-3 sm:px-4 py-2 text-center">
      ...
      <div className="mt-2 flex items-center justify-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-white/55">
